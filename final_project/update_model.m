...
lr_prev_layers = [0.02, 0.2]; % 0.02, 0.2 regularly, freezing: [0,0]
lr_new_layers  = [0.05, 0.5]; 
...
% Block 5
% input size is 64, as it is the dim of the last layer before
NEW_INPUT_SIZE  = 64;
% 5, as there are 5 classes for our subset 
NEW_OUTPUT_SIZE = 5;

net.layers{end+1} = struct('type', 'conv', ...
                           'weights', {{0.05*randn(1,1,NEW_INPUT_SIZE,NEW_OUTPUT_SIZE, 'single'),...
                           zeros(1,NEW_OUTPUT_SIZE,'single')}}, ...
...
%net.layers{end+1} = struct('type', 'relu') ;
%net.layers{end+1} = struct('type', 'conv', ...
%                           'weights', {{0.05*randn(1,1,32,32, 'single'),...
%                          zeros(1,32,'single')}}, ...
%                           'learningRate', .1*lr_new_layers, ...
%                           'stride', 1, ...
%                           'pad', 0) ;
%net.layers{end+1} = struct('type', 'relu') ;
%net.layers{end+1} = struct('type', 'conv', ...
%                           'weights', {{0.05*randn(1,1,32,NEW_OUTPUT_SIZE, 'single'),...
%                           zeros(1,NEW_OUTPUT_SIZE,'single')}}, ...
%                           'learningRate', .1*lr_new_layers, ...
%                           'stride', 1, ...
%                           'pad', 0) ;
