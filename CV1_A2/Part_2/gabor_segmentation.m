...
for jj = 1 : length(gaborFilterBank)
    % (:,:,1) is real and (:,:,2) imaginary part as defined in createGabor.m
    real_out = imfilter(img_gray, gaborFilterBank(jj).filterPairs(:,:,1), 'conv', 'replicate');
    imag_out = imfilter(img_gray, gaborFilterBank(jj).filterPairs(:,:,2), 'conv', 'replicate');
...
for jj = 1:length(featureMaps)
    ...
    featureMags{jj} = sqrt(double(real_part.^2 + imag_part.^2));
